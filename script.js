...
let extraMoney; //  Возможный доход за дополнительные работы
...
do {
amount = Number(prompt('Во сколько обойдутся обязательные расходы'));
} while (isNaN(amount));
...
do {
extraMoney = Number(prompt(`Перечислите возможный доход за ваши дополнительные работы: ${profit}?`));
...
if (getTargetMonth(purpose, accumulatedIncome) > 0) {
